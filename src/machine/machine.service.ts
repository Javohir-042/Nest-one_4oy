...
export class MachineService {
  ...
  async findOne(id: number): Promise<Machine | null> {
    const machineId = await this.machineModel.findByPk(id, { include: { all: true } })
    if (!machineId) {
  ...
  async update(id: number, updateMachineDto: UpdateMachineDto) {
    ...
    const machine = await this.machineModel.update(updateMachineDto, { where: { id }, returning: true })
    return machine[1][0];
  }
