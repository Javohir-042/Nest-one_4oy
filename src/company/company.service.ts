...
export class CompanyService {
  ...
  async create(createCompanyDto: CreateCompanyDto): Promise<Company> {
    ...
    const existsAddress = await this.companyModel.findOne({ where: { address } })
    if (existsAddress) {
  ...
  async update(id: number, updateCompanyDto: UpdateCompanyDto) {
    ...
    if (address) {
      ...
      if (existsAddress && existsAddress.id !== id) {
        throw new BadRequestException("Bunday Addres mavjud boshqa adres toping")
      }
    }
    ...
    const company = await this.companyModel.update(updateCompanyDto, { where: { id }, returning: true, })
    return company[1][0];
  }
